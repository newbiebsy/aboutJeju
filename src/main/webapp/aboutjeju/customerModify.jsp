	...
	<div class="row my-5">
		...
		<form action="customerModifyOk.jsp" class="col-10 col-sm-8 col-lg-6 col-xl-4" id="signUpfrm">
			...
			<div class="mb-3 row">
				...
				<div class="col-8">
					<select class="form-select" name="inputQuestion" id="inputQuestion">
						<%
							if(vo.getCfindpw().equals("아버지의 성함은?")){
						%>
						...
						%>
						<%
							if(vo.getCfindpw().equals("어머니의 성함은?")){
						%>
						<option value="어머니의 성함은?">어머니의 성함은?</option>
						...
						%>
						<%
							if(vo.getCfindpw().equals("부모님의 고향은?")){
						%>
						<option value="부모님의 고향은?">부모님의 고향은?</option>
						<option value="아버지의 성함은?">아버지의 성함은?</option>
						<option value="어머니의 성함은?">어머니의 성함은?</option>
						...
						%>
						<%
							if(vo.getCfindpw().equals("출신 초등학교는?")){
						%>
						<option value="출신 초등학교는?">출신 초등학교는?</option>
						<option value="아버지의 성함은?">아버지의 성함은?</option>
						<option value="어머니의 성함은?">어머니의 성함은?</option>
						...
						%>
						<%
							if(vo.getCfindpw().equals("가장 기억에 남는 선생님 성함은?")){
						%>
						<option value="가장 기억에 남는 선생님 성함은?">가장 기억에 남는 선생님 성함은?</option>
						<option value="아버지의 성함은?">아버지의 성함은?</option>
						<option value="어머니의 성함은?">어머니의 성함은?</option>
			...
			<div class="mb-3 row">
				...
				<div class="col-8">
					<input type="text" class="form-control" name="inputAnswer" id="inputAnswer" placeholder="<%=vo.getCfindpwa() %>">
				</div>
			</div>
			<div class="mb-3 row">
				<label for="inputName" class="col-form-label col-4">이름</label>
				<div class="col-8">
					<input type="text" class="form-control" name="inputName" id="inputName" placeholder="<%=vo.getCname() %>">
				</div>
			</div>
			<div class="mb-3 row">
				<label for="inputPhone" class="col-form-label col-4">휴대전화</label>
				<div class="col-5">
					<input type="number" name="inputPhone" class="form-control w-100" id="inputPhone" placeholder="<%=vo.getCphone() %>">
				</div>
